...
% --------------------------------------------------------------------------
\def\myversion{Version 0.1.0}
\def\mydate{04 November 2022}
...
\begin{center}
	\begin{table}[!h]
		...
		\begin{tabularx}{\textwidth}{| p{30mm} p{13mm} p{21mm} p{7mm} X |}
			...
			\textbf{p\_mask} [5:0]               & 48-53                     & (0x30-0x35)                     & w                   & route packets to peripheral device             \\%
...
%		\renewcommand{\arraystretch}{1.2}
	\begin{tabularx}{\textwidth}{| p{37mm} p{13mm} p{21mm} p{6mm} X |}
		...
		\cellcolor{gray!25}\textbf{reserved} & \cellcolor{gray!25}230-255 & \cellcolor{gray!25}(0xe6-0xff) & \cellcolor{gray!25} & \cellcolor{gray!25}  \\%
		\hline
	\end{tabularx}
...
\begin{center}
	...
	\begin{tabularx}{\textwidth}{| p{37mm} p{6mm} p{20mm} X |}
		...
		\textbf{dist\_key [5:0]}             & \ttfamily{32b} & \ttfamily{0x00000000} & distilling [5:0] off by default (mask = 0)  \\%
		\textbf{dist\_mask [5:0]}            & \ttfamily{32b} & \ttfamily{0x00000000} & distilling [5:0] off by default  (mask = 0) \\%
		\hline
	\end{tabularx}
	...
	\begin{tabularx}{\textwidth}{| l X |}
		...
		\textbf{notes:} & $\bullet$ \textbf{status}[31:20] reads 0x5ec when interface is up.              \\%
		                ...
		                & $\bullet$ \textbf{dist\_key[i]/dist\_mask[i]} apply to packets routed by \textbf{xp\_key[i]/xp\_mask[i]}. \\%
		\hline
	\end{tabularx}
\end{center}
			...
			\begin{algorithmic}[1]
				\For {i in [0 .. 5]}
					\If {packet routed by xp\_key[i]/xp\_mask[i]}
						\State event = dist\_key[i] \textbf{OR} (packet\_key \textbf{AND} dist\_mask[i])
					\EndIf
				\EndFor
			\end{algorithmic}
			\label{alg:distiller}
